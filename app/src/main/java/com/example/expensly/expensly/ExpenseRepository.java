...
public class ExpenseRepository {
    ...
    public boolean registerUser(String fName, String lName, String email, String password) {
        try (Cursor cursor = dbHelper.getUserByEmail(email)) {
            if (cursor != null && cursor.getCount() > 0) return false; // Email already exists
        }
        return dbHelper.insertUser(fName, lName, email, password) != -1;
    }

    public boolean loginUser(String email, String password) {
        try (Cursor cursor = dbHelper.getUserByEmail(email)) {
            if (cursor != null && cursor.moveToFirst()) {
                String storedPass = cursor.getString(cursor.getColumnIndexOrThrow(ExpenseDbHelper.COL_USER_PASS));
    ...
    public String getUserFirstName(String email) {
        try (Cursor cursor = dbHelper.getUserByEmail(email)) {
            if (cursor != null && cursor.moveToFirst()) {
                return cursor.getString(cursor.getColumnIndexOrThrow(ExpenseDbHelper.COL_USER_FNAME));
            }
        }
        return "User";
    }
    ...
    public String[] getUserName(String email) {
        try (Cursor cursor = dbHelper.getUserByEmail(email)) {
            if (cursor != null && cursor.moveToFirst()) {
                String f = cursor.getString(cursor.getColumnIndexOrThrow(ExpenseDbHelper.COL_USER_FNAME));
                String l = cursor.getString(cursor.getColumnIndexOrThrow(ExpenseDbHelper.COL_USER_LNAME));
    ...
    public boolean updateUserName(String email, String fname, String lname) {
        return dbHelper.updateUserNames(email, fname, lname) > 0;
    }

    public boolean changePassword(String email, String currentPass, String newPass) {
        if (loginUser(email, currentPass)) {
            return dbHelper.updateUserPassword(email, newPass) > 0;
        }
        return false;
    }

    public boolean updateUserPasswordForce(String email, String newPass) {
        return dbHelper.updateUserPassword(email, newPass) > 0;
    }
}
